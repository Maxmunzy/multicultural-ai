...
| 세종 | Hugging Face NLLB 기반 베트남어 번역 · 학교 특화 용어사전 구축 · 번역 검수 루프 · Edge-TTS 음성 출력 · 데이터 수집/라벨링 | `model/translation_tts/` · `data/translation_tts/` · `demo/translation_tts/` |
...
[서버]
    │ 할 일 추출 → 카테고리 분류 → 베트남어 번역 → 용어사전 검수
    ▼ POST /tts/generate
[부모 디바이스]
    체크리스트 표시 + 음성 재생
```
...
| 윤정 | (작성 예정) |
| 경이 | (작성 예정) |
| 세종 | Python, Hugging Face Transformers, facebook/nllb-200-distilled-600M, term_glossary.csv, Pandas/CSV, Edge-TTS, Gemini API(사전 확장 보조), try-except/fallback |
| 찬영 | (작성 예정) |
