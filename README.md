...
[학부모 Android 앱]
    │ 통신문 열기 → 한국어 원문 표시
    │ "AI 번역하기" 버튼 탭  ← AI 번역은 학부모가 직접 트리거
...
[FastAPI 서버 + 모델 파이프라인]
    ...
    ▼
[학부모 Android 앱]
    모국어 번역 결과(9개 중 선택) + TTS 재생(속도 조절)
    + 체크리스트 카드 + info_cards + 달력/미니달력
    + STT 음성 질문(6카테고리) + 회신 제출
```

> **AI 번역은 학부모가 트리거합니다.**  
> 선생님은 통신문을 발송하기만 하며, AI 파이프라인(`/notice/analyze`)은 학부모가 원하는 언어로 번역 버튼을 탭할 때 실행됩니다. Android 앱은 모델을 직접 실행하지 않습니다.
...
| 디자인 핸드오프 | `android/design_reference/` | UI 디자인 시안 HTML/CSS 산출물 — APK 빌드 미포함. teacher(4화면)·parent(5화면) 플로우는 실제 서비스 흐름과 일치하도록 교정됨 |
