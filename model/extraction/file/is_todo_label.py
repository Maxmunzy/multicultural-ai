...
_DEFAULT_OUTPUT = Path("model/extraction/data/train/v3.2_dual_labeled.jsonl")
...
_CAT3_SUBMISSION = re.compile(
    # 신청·참여·제출 방법 헤더 (번호 prefix 있어도 허용)
    r"(신청|참여|제출|접수|응시)\s*방법\s*[:：]"
    r"|(신청|참여|접수)\s*링크\s*[:：]"
    # 참석·불참 체크 형식
    r"|참석\s*(또는|혹은)\s*불참"
    # 서류 + 제출 동사
...
_CAT6_MISC = re.compile(
    ...
    r"급식\s*(여부|없음|있음|신청|미제공|없이|중단|제공\s*(여부|없음|있음))"
