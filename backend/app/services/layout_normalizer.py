...
2. **헤더-값은 콜론(:) 형식** — 조사("은/는") 사용 X
   ❌ "신청방법은 ..." / "비용은 ..." → ✅ "신청방법: ..." / "비용: ..."
3. **한 sentence에 같은 헤더 키워드 두 번 X** — ❌ "준비: ...개인 준비물 등입니다"
4. **표 행은 한 줄 sentence** — 분류·구분 정보(학년/공용·개인/대상 등)는 sentence 끝 괄호로 보존:
   ✅ "준비물: 알림장, 클리어 화일 (1학년 공용)"
   ✅ "운영시간: 오전 10시 (1-3학년)"
   ✅ "비용: 35,000원 (4학년 학부모)"
   ❌ "1학년 공용 준비물: ..." — 분류 정보 prefix 시작 금지
5. **특수기호 → 알파벳 변환** (윤정 모델 _clean_symbols 호환):
   ○ → O, ✕ → X, □ → [], ✓ → V, ☑ → [V]
   (마크업 ■, ※, ▶ 등은 그대로 보존)
6. **자간 공백만 정상화** ("학 년 도" → "학년도", "의 정 부 시" → "의정부시"), 다른 글자 변경 X
7. **종결어미 강제 X** — 원문 그대로
8. **요약·번역·축약 금지** — 원문 표현 그대로
9. **날짜·시간·금액·URL·전화번호·고유명사·학교명·지명 원문 그대로**
10. **단독 기호 줄(■■■, 가로줄)만 제거** — 텍스트와 같이 있는 마크업은 보존
11. **원문에 없는 정보 추측·추가 금지**
