"""CRF 학습 노트북 생성 — Hybrid + CRF layer (sequence consistency).

v6 결과 부족 시 fallback axis.
- use_crf=True
- pytorch-crf 의존성 추가
- 학습 + Drive 백업
"""
...
md("""# SchoolBridge — Hybrid + CRF 학습 (단편화 fix axis)

**CRF 추가**:
- BIO head 위에 CRF layer (전체 sequence transition matrix 학습)
- "O 다음 갑자기 I" 같은 inconsistent 패턴 자동 억제 → 단편화 ↓
- NER에서 검증된 표준 트릭 (BERT-CRF)

**전제**:
- v6 학습 데이터 그대로 사용 (layoutxlm_bio_train.jsonl)
- hybrid_model.py 에 use_crf=True 옵션 적용
...
best_val = float('inf')
DRIVE_PATH = '/content/drive/MyDrive/hybrid_v7_crf_best.pt'
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_val:
        ...
        try:
            shutil.copy('hybrid_crf_best.pt', DRIVE_PATH)
...
out = Path("sentence_extraction/colab_upload_hybrid/hybrid_train_colab_crf.ipynb")
